...
public class LocationService {
    ...
    public Location postLocation(Long dayPlanId, String addressName, String kakaoMapId, String phone, String placeName, String placeUrl, Float lat, Float lng, String roadAddressName, String imgUrl, String CategoryGroupName, Long planId, Member member) {
        ...
        if(dayPlan.getPlan().getId().equals(planId) && dayPlan.getPlan().getPlanMembers().contains(member)) {
            ...
            throw new RuntimeException("plan member가 아닙니다.");
        }
        //저장된 플랜 반환
    }

    // Location 수정
    public Location updateLocation(Long locationId, String addressName, String kakaoMapId, String phone, String placeName, String placeUrl, Float lat, Float lng, String roadAddressName, String imgUrl, String CategoryGroupName, Long planId, Member member) {

        // 기존 Location 검색
        Location location = locationRepository.findById(locationId)
                .orElseThrow(() -> new RuntimeException("Location not found with id: " + locationId));
        if(location.getDayPlan().getPlan().getId().equals(planId) && location.getDayPlan().getPlan().getPlanMembers().contains(member)) {
            // 수정된 정보 업데이트
    ...
    // Location 삭제
    public void deleteLocation(Long locationId, Long planId, Member member) {
        // Location 검색
        Location location = locationRepository.findById(locationId)
                .orElseThrow(() -> new RuntimeException("Location not found with id: " + locationId));
        // 삭제
    ...
    // 장소 찜하기
    public Location postWishLocation(Long planId, String addressName, String kakaoMapId, String phone, String placeName, String placeUrl, Float lat, Float lng, String roadAddressName, String imgUrl, String CategoryGroupName, Long PathplanId, Member member) {

        // Plan 검색
        Plan plan = planRepository.findById(planId)
                .orElseThrow(() -> new RuntimeException("Plan not found with id: " + planId));
        if(plan.getId().equals(planId)&& plan.getPlanMembers().contains(member)) {
            // Location 저장
    ...
    // 찜한 장소를 정식 등록
    public Location registerWishLocation(Long locationId, Long dayPlanId, Long planId, Member member) {
